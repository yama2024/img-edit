...
**重要: 図形とテキストの重なり順序を自由に変更できる完全なレイヤー管理システムを実装**

#### レイヤー操作の種類
- **最前面へ (bringToFront)**: `app.js:467-486` - 選択中のテキスト/図形を配列の最後に移動
- **前面へ (bringForward)**: `app.js:509-538` - 選択中のテキスト/図形を1つ上（配列インデックス+1）に移動
- **背面へ (sendBackward)**: `app.js:540-569` - 選択中のテキスト/図形を1つ下（配列インデックス-1）に移動
- **最背面へ (sendToBack)**: `app.js:488-507` - 選択中のテキスト/図形を配列の最初に移動

#### レイヤーシステムの仕組み
- **配列の順序 = 描画順序**: テキストと図形は配列の順序で描画される
  - 配列の前（インデックスが小さい）→ 背面に描画
  - 配列の後（インデックスが大きい）→ 前面に描画
- **独立した管理**: テキスト（`textObjects[]`）と図形（`shapeObjects[]`）は独立して管理
- **選択状態の更新**: レイヤー移動後、`selectedTextIndex`または`selectedShapeIndex`を更新

#### 主要機能
- **自動再描画**: レイヤー変更後、`redrawCanvas()`で即座に反映
- **Undo/Redo対応**: すべてのレイヤー操作は`saveState()`で履歴に保存
- **リアルタイム通知**: 操作の成功/失敗を視覚的に表示
- **範囲チェック**: 既に最前面/最背面の場合は通知を表示

#### UI実装
- **レイヤーセクション**: `index.html:199-205` - 4つのボタンを配置
- **ツールチップ**: 各ボタンに詳細な説明を表示
- **ARIAラベル**: スクリーンリーダー対応
...
#### 操作フロー
1. **テキストまたは図形を選択** → クリックで選択状態にする
2. **レイヤーボタンをクリック** → 最前面へ/前面へ/背面へ/最背面へ
3. **配列操作** → splice/unshift/pushまたは要素入れ替え
4. **再描画** → redrawCanvas()で新しい順序で描画
5. **履歴保存** → saveState()でUndo可能に
6. **通知表示** → 操作結果をユーザーに通知

### 9. UI/UXの改善（プロフェッショナル品質）
