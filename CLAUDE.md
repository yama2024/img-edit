...
### 2. 描画ツール
- **ブラシツール**: `app.js:235-251` - 自由描画（画像ロード後のみ）
- **消しゴムツール**: `app.js:235-251` - 白色で上書き消去（画像ロード後のみ）
- **画像ロードチェック**: `app.js:230-233` - 画像がロードされていない場合は警告表示
- **色選択**: `app.js:66-68` - カラーピッカーで色を変更
- **線の太さ調整**: `app.js:71-74` - スライダーで1-50px調整

### 3. テキスト入力
- **テキスト追加**: `app.js:261-275` - クリック位置にテキストを配置
- **画像ロードチェック**: `app.js:263-266` - 画像がロードされていない場合は警告表示
